...
#         import dill
    ...
    def _run(self,
        ...
        for i, batch in enumerate(loader):
            ...
            loss, batch_results, batch_postfix = self.batch_pass(device, batch, *args, **kwargs)
            self.losses.append(float(loss.data))

            
            pbar_postfix = {'loss': float(loss.data), 'avg_loss': sum(self.losses)/len(self.losses)}  # 'epoch': self.epoch, 
            pbar_postfix.update(batch_postfix)
            if tqdm_bar:
        ...
        single_num_score, results = self.agg_results(self.raw_results)
        ...
        for train_epoch in range(train_epochs):
            ...
            if self.seed is not None:
                with set_temp_seed(self.seed):
                    train_loss, train_score, train_results = self.run(device, train_loader, tqdm_bar=tqdm_bar, max_iterations=max_iterations_train, *args, **kwargs)
            else:
                train_loss, train_score, train_results = self.run(device, train_loader, tqdm_bar=tqdm_bar, max_iterations=max_iterations_train, *args, **kwargs)
            self.optimizer.zero_grad()
            ...
            if save:
                ...
                if epochs_save is not None and epochs_save > 0 and self.epoch % epochs_save == 0:
            ...
            if prints == 'display':
                ...
                print('epoch {:3d}/{:3d} | train_loss {:.5f} | val_loss {:.5f} | train_score {:.5f} | val_score {:.5f} | train_time {:6.2f} min{:}'
                      .format(self.epoch, train_epochs + start_epoch, train_loss, val_loss, train_score, val_score, train_time, " | best" if best else ""))
    ...
    def predict(self,
        ...
        with torch.no_grad():
            ...
            for batch in loader:
                # pass batch through model, calculate loss and predict
                _, batch_results, _ = self.batch_pass(device, batch, *args, **kwargs)
...
#                 self.raw_results[result] = np.concatenate(self.raw_results[result])
    ...
    def evaluate(self,
        ...
        with torch.no_grad():
            self.model.train(False)
            self.train_data = False
            loss, score, results = self.run(device, loader, tqdm_bar=tqdm_bar, *args, **kwargs)
